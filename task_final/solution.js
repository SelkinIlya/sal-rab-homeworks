...
function sendRequest(name, phone, address, goods, sum) {
    ...
    all_data.data.client = `${name} ${phone}`;
    all_data.data.order.address = `ул. ${address.street}, дом ${address.house}, ${address.entrance} подъезд, ${address.floor} этаж, кв ${address.flat}`;
    all_data.data.order.sum = sum;

    for (let i = 0; i < goods.length; i++) {
        all_data.data.goods.push({title: goods[i].title, count: goods[i].count});
    }
